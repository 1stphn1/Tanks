...
namespace Game {
    Player player;
    Bot bot;
}  // Declaring these variables in the Game.h file throws this error: 'Player' does not name a type. So, I declared them here, and it works.
...
int  Game::width;
int  Game::height;
int  Game::terrain[WINDOW_WIDTH];
int  Game::player_x, Game::player_y;
int  Game::bot_x, Game::bot_y;

SDL_Window   *Game::window;
SDL_Renderer *Game::renderer;
SDL_Texture  *Game::background_texture;
SDL_Texture  *Game::ground_texture;
SDL_Texture  *Game::surfacelay_texture;
SDL_Texture  *Game::tank_texture;
SDL_Texture  *Game::barrel_texture;
SDL_Texture  *Game::projectile_texture;
...
bool Game::InitGame(const char *window_title)
{
    ...
    window = SDL_CreateWindow(window_title, 200, 200, width, height, SDL_WINDOW_FULLSCREEN);
...
void Game::RenderMap()
{
    ...
    for (int i = 0; i < WINDOW_WIDTH; i++) {  // Renders surface layer
        ...
        if (i != 0 && ( (WINDOW_HEIGHT - terrain[i - 1]) - (WINDOW_HEIGHT - terrain[i]) <= 5  && (WINDOW_HEIGHT - terrain[i - 1]) - (WINDOW_HEIGHT - terrain[i]) >= -5 )) {
...
void generateTerrain() 
{
    ...
    for (int i = 0; i < WINDOW_WIDTH; i++) {
        ...
        Game::terrain[i] = std::round(lerp(0, total, WINDOW_HEIGHT / 2) * range);
    }
}
